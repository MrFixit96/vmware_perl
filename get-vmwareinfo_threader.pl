...
sub Worker_Thread(){
	...
	if ($search){
		# if the host flag is set include it in the command string, else skip it.
		if (!$host){
			#Use a system call to kick off the original single threaded single input script as a worker thread
			system("/home/admin/esx/get-vmwareinfo.pl --find $search  --show_location --show_resources --output_csv $ofname");
		}else{
			#Use a system call to kick off the original single threaded single input script as a worker thread
			system("/home/admin/esx/get-vmwareinfo.pl --host --find $search  --show_location --show_resources --output_csv $ofname");
		}#endIF
	}#endIF
}#endIF
